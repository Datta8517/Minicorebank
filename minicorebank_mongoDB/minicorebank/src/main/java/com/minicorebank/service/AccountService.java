...
public class AccountService {
    ...
    public Account createAccount(String userId, AccountRequest req) {
        User user = userRepository.findById(userId).orElseThrow(() -> new RuntimeException("User not found"));

        // Check if the user already has this type of account
        boolean exists = accountRepository.existsByUserIdAndType(user.getId(), req.getType().toUpperCase());
    ...
    public Account checkAccount(String loginId, TransactionRequest request){
        Account account = accountRepository.findById(request.getAccountId())
                .orElseThrow(() -> new RuntimeException("Account not found"));

        if(!account.getUserId().equals(loginId)) {
    ...
    public void deposit(String userId, TransactionRequest req) {
        ...
        tr.setType("DEPOSIT");
        tr.setUserId(account.getUserId());
    ...
    public void withdraw(String userId, TransactionRequest req) {
        Account account = checkAccount(userId, req);
    ...
    //admin only
    public String approveAccount(String accountId) {
        Account acc = accountRepository.findById(accountId)
                .orElseThrow(() -> new RuntimeException("Account not found"));
        User user =  userRepository.findById(acc.getUserId()).orElseThrow(()-> new RuntimeException("User not Found"));
