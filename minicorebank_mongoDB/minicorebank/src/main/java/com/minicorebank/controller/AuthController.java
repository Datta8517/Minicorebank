...
//            String token = jwtService.generateToken(user);
            ...
            refreshToken.setExpiresAt(
                    jwtService.extractAllClaims(refreshTokenValue).getExpiration().toInstant()
            );
    ...
    public ResponseEntity<AuthResponse> refresh(@RequestBody RefreshTokenRequest request) {
        ...
        User user = userService.findById(rt.getUserId()).orElseThrow(()-> new RuntimeException("null"));
...
//            if (jwtBlacklistRepository.existsByToken(accessToken)) {
//                // already logged out → silently ignore
