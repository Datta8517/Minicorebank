...
public class TransactionController {
    ...
    public ResponseEntity<TransactionRecord> transfer(@RequestHeader("Authorization") String authHeader, @RequestBody TransferRequest req, HttpServletRequest request) {
        TransactionRecord tr = transactionService.transfer(authHeader, req.getFromAccountId(), req.getToAccountId(), req.getAmount(), request);
        return ResponseEntity.ok(tr);
    }

    @GetMapping("/account/{accountId}")
    public ResponseEntity<List<TransactionRecord>> accountTxs(@PathVariable Long accountId) {
